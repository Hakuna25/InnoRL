...
class PPOAgent(Agent):
    ...
    def update(self, replay_buffer, total_steps):
        ...
        """
        s = torch.reshape(s, (-1, s.shape[-1])).to(device)
        a = torch.reshape(a, (-1, a.shape[-1])).to(device)
        a_logprob = torch.reshape(a_logprob, (-1, a_logprob.shape[-1])).to(device)
        r = torch.reshape(r, (-1, r.shape[-1])).to(device)
        s_ = torch.reshape(s_, (-1, s_.shape[-1])).to(device)
        dw = torch.reshape(dw, (-1, dw.shape[-1])).to(device)
        done = torch.reshape(done, (-1, done.shape[-1])).to(device)
