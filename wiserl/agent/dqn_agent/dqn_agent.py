...
from wiserl.net.nn_net import QNet, DQNNet
...
class DqnAgent(Agent):
    def __init__(self,  config , sync=True):
        ...
        self.actor = DQNNet(self.config.state_dim, self.config.action_dim).to(device)
        self.actor_target = DQNNet(self.config.state_dim, self.config.action_dim).to(device)
    ...
    def update(self, s, a, r, s_, done):
        ...
        q_target = b_r + self.config.gamma * q_next.max(1)[0].view(-1, 1) # (batch_size, 1)
